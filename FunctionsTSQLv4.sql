...
-- SELECT COALESCE(NULL, 'x', 'y', '123') --Возвращает первое значение, не равное null (сокращенный аналог CASE для проверки на null), вернет x
		...
		--COALESCE и ISNULL(то же самое, но принмает 2 аргумента) не рекомендуется использовать при поиске
		--так как они манипулируют атрибутами => SQL Server не может обеспечить упорядочение индекса, то есть доступные индексы
		--используются неэффективно. Напрмер вместо COALESCE(T1.col1, -1) = COALESCE(T2. col1, -1) лучше использовать способ без
		--манипуляции атрибтами: T1.col1 = T2.col1 OR (T1.col1 IS NULL AND T2.col1 IS NULL). Ее SQL Server понимает как просто сравнение, 
		--которое рассматривает пустые значения как равные. С помощью этой формы SQL Server может эффективно использовать индексацию
		--ДОПОЛНЕНИЕ:
		--col1 - 1 <= @n и col1 <= @n + 1 тоже влияют по-разному, в первом случае идет манипулирование атрибутом => могут неэффективно
		--использоваться индексы, во втором случае манипулирования нет и это выражения является АРГУМЕНТОМ поиска, следовательно
		--индексы могут использоваться эффективно
